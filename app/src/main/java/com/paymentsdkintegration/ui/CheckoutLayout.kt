package com.paymentsdkintegration.ui
...
import com.paymentsdkintegration.R
import com.paymentsdkintegration.utilities.googleWalletPayApi.PaymentUiState
import com.paymentsdkintegration.utilities.googleWalletPayApi.PaymentsUtils
...
fun ProductScreen(
    ...
    samsungPayReady: Boolean = false,
    onSamsungPayButtonClick: () -> Unit,
) {
    val padding = 20.dp
        ...
        Column(
            ...
            if (payUiState !is PaymentUiState.NotStarted) {
                PayButton(
                    ...
                    allowedPaymentMethods = PaymentsUtils.allowedPaymentMethods.toString()
                )
            }
            if (samsungPayReady) {
                Button(
                    modifier = Modifier
                        .testTag("payButton")
                        .fillMaxWidth(),
                    onClick = onSamsungPayButtonClick,
                    enabled = samsungPayReady,
                    content = {
                        Text(
                            text = "Samsung Pay",
                            color = black
